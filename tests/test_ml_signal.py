...
def test_ml_signal_risk_penalty_reorders_by_final_score(temp_models_dir):
    """测试风险惩罚会将排序从 ml_score 切换到 final_score。"""
    registry = ModelRegistry(models_dir=temp_models_dir)
    version = registry.register_model(
        model=MockMLModel(),
        model_type="xgboost",
        ...
        risk_penalty_config={
            "enabled": True,
            "penalty_lambda": 0.2,
            "feature_weights": [{"name": "spec_score", "weight": 1.0}],
        },
    )

    signal = MLSignal(top_n=1, model_version=version, models_dir=temp_models_dir)
    date = pd.Timestamp("2023-06-15")
    universe = ["000001.SZ", "000002.SZ", "000003.SZ"]
    features_df = pd.DataFrame(
        {
            ...
            "f2": [1.0, 2.0, 3.0],
            "f3": [4.0, 5.0, 6.0],
            "spec_score": [100.0, 1.0, 10.0],
        }
    )

    signals = signal.generate(date, universe, {"features": features_df})

    assert list(signals.keys()) == ["000002.SZ"]


def test_ml_signal_generate_ranked_uses_final_score_when_risk_penalty_enabled(temp_models_dir):
    """测试 generate_ranked 在启用风险惩罚后返回 final_score 排序。"""
    registry = ModelRegistry(models_dir=temp_models_dir)
    version = registry.register_model(
        model=MockMLModel(),
        model_type="xgboost",
    ...
    signal = MLSignal(top_n=3, model_version=version, models_dir=temp_models_dir, verbose=False)
    date = pd.Timestamp("2023-06-15")
    universe = ["000001.SZ", "000002.SZ", "000003.SZ"]
    features_df = pd.DataFrame(
        {
            ...
            "f3": [4.0, 5.0, 6.0],
            "spec_score": [100.0, 1.0, 10.0],
            "amount_ma20": [60000.0, 60000.0, 60000.0],
            "total_mv": [1000000.0, 1000000.0, 1000000.0],
            "sw_l1_code": ["801010", "801010", "801010"],
        }
    )

...
def test_default_model_loader_suppresses_pickle_warning(monkeypatch):
    """折模型默认加载器抑制 XGBoost 跨版本反序列化告警（纸面日志噪音）。"""
    ...
    model = TerminalLossModel(
        TerminalLossModelConfig(task_id="t", feature_names=[]), classifier=object()
    )
    msg = (
    ...
    with warnings.catch_warnings():
        ...
        assert ps._default_model_loader("dummy.joblib") is model

...
class TestDataCleaner:
    ...
    def test_clean_daily_sorting(self, cleaner, mock_daily_raw, mock_adj_factor_raw):
        ...
        assert list(result[["ts_code", "trade_date"]].itertuples(index=False, name=None)) == sorted(
            result[["ts_code", "trade_date"]].itertuples(index=False, name=None)
        )
    ...
    def test_add_tradable_universe_flag_stock_st_override(self, cleaner):
        ...
        stock_st_df = pd.DataFrame(
            {"ts_code": ["000001.SZ"], "trade_date": ["20230110"], "is_st": [1]}
        )
    ...
    def test_validate_uniqueness_success(self, cleaner):
        """测试唯一性验证（成功）"""
        df = pd.DataFrame({"key1": ["A", "B", "C"], "key2": [1, 2, 3]})
    ...
    def test_validate_uniqueness_failure(self, cleaner):
        """测试唯一性验证（失败）"""
        df = pd.DataFrame({"key1": ["A", "A", "B"], "key2": [1, 1, 2]})
    ...
    def test_clean_daily_with_missing_adj_factor(self, cleaner):
        ...
        adj_factor_raw = pd.DataFrame(
            {"ts_code": ["000001.SZ"], "trade_date": ["20230102"], "adj_factor": [1.1]}
        )
        ...
        # 验证计算
        assert (
            abs(result[result["ts_code"] == "000001.SZ"]["close_adj"].iloc[0] - 10.0 * 1.1) < 0.01
        )

    def test_clean_daily_rejects_all_invalid_adj_factor(self, cleaner):
        """复权因子全无有效值时拒绝生成可缓存的 clean 数据。"""
        daily_raw = pd.DataFrame(
            {
                "ts_code": ["000001.SZ"],
                "trade_date": ["20230102"],
        ...
        adj_factor_raw = pd.DataFrame(
            {"ts_code": ["000001.SZ"], "trade_date": ["20230102"], "adj_factor": [float("nan")]}
        )

        with pytest.raises(ValueError, match="没有任何有效数值"):
            cleaner.clean_daily(daily_raw, adj_factor_raw)

...
def test_suppress_xgboost_pickle_warning_serialized_model():
    """跨版本 pickle 反序列化告警同样被精确屏蔽（公共模块入口）。"""
    from src.lazybull.common.xgboost_compat import suppress_xgboost_pickle_warning
    ...
    # 上下文外：警告正常发出（验证匹配正则可用）
    with pytest.warns(UserWarning, match="loading a serialized model"):
        warnings.warn(msg, UserWarning)
...
def test_suppress_xgboost_pickle_warning_single_source():
    """旧入口（model_registry）与公共模块为同一实现（单一来源）。"""
    from src.lazybull.common.xgboost_compat import (
        suppress_xgboost_pickle_warning as common_suppressor,
    )
    from src.lazybull.ml.model_registry import _suppress_xgboost_pickle_warning

    assert _suppress_xgboost_pickle_warning is common_suppressor

...
from src.lazybull.common.sidecar_schema import SNAPSHOT_COLUMNS_ZH
...
class TestWalkForwardSplits:
    ...
    def test_quarterly_splits_generation(self, trade_cal):
        ...
        # 3年大约是1095天（允许误差）
        assert (
            900 < train_duration_days < 1200
        ), f"训练窗口应约为3年，实际为 {train_duration_days} 天"
    ...
    def test_tree_limited_model_setstate_rebuilds_base_model_from_legacy_state(self, monkeypatch):
        ...
        monkeypatch.setattr(
            TreeLimitedModel, "_rebuild_base_model_from_legacy_state", staticmethod(fake_rebuild)
        )
...
class TestWalkForwardCSV:
    ...
    def test_write_walk_forward_trade_details(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            trades_path = os.path.join(tmpdir, "walk_forward_trades_wf_test_trade_split06.csv")
    ...
    def test_write_walk_forward_holdings_snapshot(self):
        """持仓快照导出必须用**中文表头**（政策旁路统一约定）。"""
        with tempfile.TemporaryDirectory() as tmpdir:
            from scripts.walk_forward import write_walk_forward_holdings_snapshot
                ...
                [
                    {
                        ...
                        "holding_days": 1,
                        "planned_exit_date": pd.Timestamp("2024-01-08"),
                        "remaining_intervals": 3,
                    }
                ]
            )
            results = [{"split_index": 6, "model_version": 123, "_holdings_snapshot": snapshot}]
            summary_path = os.path.join(tmpdir, "walk_forward_summary_test.csv")

            write_walk_forward_holdings_snapshot(results, summary_path, "wf_test_hold")

            path = os.path.join(tmpdir, "walk_forward_持仓快照_wf_test_hold_split06.csv")
            ...
            assert list(exported.columns) == [
                ...
                "折序号",
                "模型版本",
                *SNAPSHOT_COLUMNS_ZH.values(),
            ]
            assert exported.loc[0, "运行标识"] == "wf_test_hold"
            assert exported.loc[0, "股票代码"] == "000001.SZ"
            assert exported.loc[0, "剩余持有交易日"] == 3
            assert exported.loc[0, "日期"] == "2024-01-04"
    ...
    def test_write_walk_forward_topk_details(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            export_path = os.path.join(tmpdir, "walk_forward_topk_details_wf_test_777_split03.csv")

...
- 问题根因：`REALTIME_SNAPSHOT_TIMEOUT_SECONDS` 默认 240 秒（4 分钟），导致 UI 被慢数据源（efinance/akshare 需 3-5 分钟）阻塞
- **解决方案**：缩短超时到 10 秒，让 UI 快速超时并用缓存数据（30 分钟有效期）渲染
- 后台线程继续异步获取新数据，缓存回退逻辑已就位，确保不会展示过时数据超过 30 分钟
- 结果：即使数据源需要 3-5 分钟，用户也不会看到 3-4 分钟卡屏，显示流畅

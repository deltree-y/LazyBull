...
- **report_rc 下载与身份闭环**：单日增量及以目标交易日为锚的近六年回补均按 2000 行分页，年度累计超过 10 万行时自动按日期二分，当前年度分区从最新研报日次日续传；离线/在线统一使用股票、日期、机构、作者、标题标识研报并以 quarter 标识预测行，加载、去重或构建因子遇到旧弱 schema 均会明确要求强制重下。
- **一致预期特征改用经济可比口径**：新增 FY-1，训练使用 FY-1/FY0/FY1/FY2 预测 EPS 收益率及目标价上行空间；基础与修正状态均最多保留 365 日并由 freshness 平滑衰减。
- **一致预期修正因子 v2 重做**：修正因子改用 `eps` 列并按绝对预测财年分组（锚定年 FY1 优先），分歧度为同日同财年研报级、修正速度为真实日历天数斜率、评级上调比例真实读取 `rating`；基线统一为近 30 日 vs 此前 90 日且两侧窗口不重叠，输出截面 winsorize，并以恒写入版本号的 `cons_revision_schema_v2` 哨兵列强制旧语义缓存重建。
- **训练契约可追溯**：基础与修正一致预期开关启用时缺少构建列或哨兵列不完整会明确失败，split/deploy 注册元数据均保存全部特征开关；WF summary 记录门禁后实际存活列，skip-training 核验旧模型开关一致性。

### 升级说明

- **v0.96.1**：启用 `--enable-consensus-revision-features` 的区间需重建 `cs_train` / `cs_infer`（旧缓存缺哨兵列会自动触发重建），无需重下 raw；旧模型（如 v22715）同名 `zscore_cons_*` 列会静默读到 v2 语义，建议停用或以重建后特征重新训练。
- **v0.96.0**：首次升级需以 `--download report_rc --force` 重下目标训练区间，再重建特征并重训；此后可正常增量续传。

### v0.95.14

- **回归测试与代码行为对齐**：`daily` 接口级限频 480 与“盘后优先日线快照”均为既有有意行为，同步更新相关测试断言；全量 1265 个测试全部通过。

- **季度公告加载告警降噪**：现金流等季度公告数据补充窗口前历史记录时，统一精确屏蔽 pandas empty/all-NA concat 的 `FutureWarning`；不删除数据或全 NA 列，不改变 dtype 推断输入与加载顺序。

### v0.95.12

- **业绩快报迁移告警降噪**：旧 express 单文件与已有季度分区合并时，仅精确屏蔽 pandas empty/all-NA concat 的 `FutureWarning`，不删除全 NA 数据或列，不改变 raw schema 与迁移去重结果。

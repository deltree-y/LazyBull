...
- 新增 3 大另类因子模块，三者各自配备独立开关（默认关闭，保持基线行为）
  ...
  - **龙虎榜（lhb）**：`top_list` 个股日频上榜数据，同日多次上榜优先取单日榜（净买入绝对值最大的一条，仅连续类理由时保留），按交易日历重采样计算近 5/20 日滚动净额与 20 日上榜次数（未上榜日保留历史累计衰减）；新增 `lhb_cont_on_list`（当日是否因"连续异动"类理由上榜，reason 含"连续"，事件级信号）；近期空响应不落盘延迟重试、近期空占位自动重新查询防假空

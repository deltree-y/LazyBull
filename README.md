...
- **一致预期特征改用经济可比口径**：新增 FY-1，训练使用 FY-1/FY0/FY1/FY2 预测 EPS 收益率及目标价上行空间；基础与修正状态均最多保留 365 日并由 freshness 平滑衰减，修正目标价使用独立列和未复权收盘价，不再覆盖基础因子。
- **训练契约可追溯**：基础与修正一致预期开关启用时缺少构建列会明确失败，split/deploy 注册元数据均保存全部特征开关。首次升级需以 `--download report_rc --force` 重下目标训练区间，再重建特征并重训；此后可正常增量续传。

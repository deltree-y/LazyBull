...
**涨跌停与停牌处理**:
- **选股阶段过滤**: 自动过滤停牌、涨停股票，避免无法交易的股票进入候选池
- **延迟订单机制**: 遇到涨跌停/停牌不会直接失败，而是加入延迟队列定期重试
- **可配置参数**: 支持开关延迟功能（`enable_pending_order`），配置最大重试次数和延迟天数
- **详细日志**: 记录过滤、延迟、重试的完整信息，包含股票代码、原因、价格等
- **向后兼容**: 默认启用但不破坏现有API，所有现有测试保持通过
- **完整测试**: 新增30个单元测试和4个集成测试验证功能正确性
...
- [涨跌停与停牌处理指南](docs/trade_status_guide.md): 涨跌停与停牌状态的自动处理机制（新增）

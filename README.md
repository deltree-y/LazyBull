...
- ✅ **横截面标签处理**: **默认训练流程支持按日横截面去极值+标准化，提升IC稳定性**（新增）
- ✅ **IC统计增强**: **按日IC序列统计（IC_mean/std/IR，分位数），全面评估模型排序能力**（新增）

### v0.4.0 更新内容（2026-01-24）

**横截面标签处理与IC统计增强** - 提升模型IC/RankIC的稳定性和可诊断性：

#### 核心功能

1. **按日横截面标签处理（默认启用）**
   - **去极值（Winsorize）**: 每个交易日内独立截断上下1%极端值，减少异常值影响
   - **标准化（Z-score）**: 每个交易日内独立标准化到均值0、标准差1，消除跨期尺度差异
   - **无未来泄漏**: 每日独立处理，不使用未来日期信息
   - **自动集成**: 在 `prepare_training_data` 中自动应用，无需额外配置
   - **只用于排序**: 标签标准化后直接用于训练，无需还原到收益率尺度

2. **IC评估增强**
   - **按日IC序列**: 计算每个交易日的IC和RankIC，追踪时序稳定性
   - **统计指标**: IC_mean（均值）, IC_std（标准差）, IC_IR（信息比率）
   - **RankIC统计**: Rank_IC_mean, Rank_IC_std, Rank_IC_IR（对选股更重要）
   - **分位数分析**: IC分位数（P10/P50/P90）诊断IC分布
   - **胜率统计**: IC>0的比例，衡量稳定性
   - **格式化报告**: 自动打印详细的IC统计报告

3. **新增模块**
   - **`src/lazybull/ml/preprocess.py`**: 通用预处理工具
     - `cross_sectional_winsorize()`: 横截面去极值
     - `cross_sectional_zscore()`: 横截面标准化
     - `process_labels_cross_sectional()`: 完整处理流程
     - `validate_cross_sectional_standardization()`: 验证处理效果
   
   - **`src/lazybull/ml/evaluation.py`**: IC评估工具
...
4. **训练脚本增强**
   - 修改 `scripts/train_ml_model.py`：
     - 默认启用横截面标签处理
     - 增强验证集评估输出（按日IC统计）
     - 移除旧的标签winsorize（改为横截面处理）
     - 添加日期参数传递
...
**横截面处理原理**：
- 每个 trade_date 分组独立处理
- 先 winsorize 截断极端值（减少噪音）
- 再 zscore 标准化（消除尺度差异）
- 处理后每日均值≈0，标准差≈1

**为什么有效**：
- **减少异常值干扰**: 涨跌停、停牌复牌等极端收益被截断
- **消除跨期差异**: 牛市/熊市标签尺度差异被标准化
- **提升排序能力**: 模型关注相对排序而非绝对值
- **稳定IC**: 各日标签分布一致，IC更稳定

**指标解读**：
- **IC > 0.03**: 有一定预测能力
- **RankIC > 0.05**: 排序能力较好（选股关键）
- **IR > 1.0**: IC稳定性好
- **胜率 > 55%**: 大多数时间IC为正
...
新增测试文件：
- `tests/test_ml_preprocess.py`: 测试横截面处理
  - 验证每日均值≈0，标准差≈1
  - 验证 winsorize 截断生效
  - 测试边界情况（缺失值、标准差为0等）

- `tests/test_ml_evaluation.py`: 测试IC统计
  - 验证按日IC计算正确
  - 验证统计指标计算
  - 测试边界情况（样本不足、完美相关等）
...
- **兼容性**: 完全向后兼容，现有代码无需修改
- **替代关系**: 横截面处理替代了旧的全局 winsorize
- **增强关系**: IC统计增强了原有的简单IC/RankIC输出
- **默认启用**: 作为新的默认训练流程，无需手动配置

#### 预期效果

根据项目文档 `docs/ic_optimization_guide.md` 的建议：
- **短期目标**: IC从0.1提升到0.15+
- **中期目标**: IC提升到0.20+，IR>1.0
- **稳定性**: IC_std降低，IC胜率提升

...
- **walk-forward 对比报表新增三视角评分体系**：
  - `wf_comparison*.xlsx` 新增 `模型Alpha评分`、`交易参数收益评分`、`实盘候选评分` 三张工作表。
  - `模型Alpha分` 聚焦选股统计，解答“哪个模型/超参更好”。
  - `交易收益分` 在相同模型参数 + 相同时间段内做交易参数配对百分位，再跨模型、跨时间段平均，避免强模型污染交易参数判断。
  - `实盘候选分` 综合模型 Alpha、交易收益、交易稳健与最差场景防守，并加入硬门槛；不满足门槛的组合直接置 0。
  - Excel 中关键得分列与参与因子按权重着色，0 分候选的失败原因列用浅红色标识。
...
- **持有期拖尾提前调仓补全权重校验，与回测侧行为严格一致**：
  在 `src/lazybull/paper/runtime.py` 的 `_execute_t0_if_rebalance_day()` 中，
  `holding_tail` 模式现于 T0 生成指令后执行权重 post-check：
  `残留仓位占比 + 新信号买入权重 ≤ 100%`，超限则清空指令并返回 `not_rebalance_day`。
  与回测侧 `engine.py` 第 825-840 行的"先生成信号→校验权重→超限撤回"逻辑完全对齐。

### Test

- 新增 `test_holding_tail_post_check_rejects_when_weight_exceeds`（超限→撤回）
  和 `test_holding_tail_post_check_allows_when_weight_ok`（未超限→允许）
  两条回归用例，直接验证权重约束。
...
- **SMB 远端读取**：基于系统 `smbclient` 命令实现（SMB2/3 协议，可靠性优于 Python SMB 库）
...
- **回测非动态持仓分支补齐 T0->T1 卖后买链路**：在 [src/lazybull/backtest/engine.py](src/lazybull/backtest/engine.py) 中，`enable_profit_based_holding=False` 且触发 `holding_period` 卖出时，现会同步记录卖出槽位权重并写入次日补位买入计划；T+1 可按“先卖后买”执行，不再出现“T+1 只卖不买 -> 空仓触发提前调仓 -> 轮次被压缩”的错位行为。

### 测试

- 更新 [tests/test_backtest_t1.py](tests/test_backtest_t1.py)，新增 `test_holding_period_sell_generates_refill_buy_on_t1_without_profit_mode` 回归用例，覆盖“未启用盈亏动态持仓时，持有期卖出后次日先卖再补买”的路径。

## [0.72.5] - 2026-05-30

### 优化

- **调仓决策摘要“最终”段精简为短标签**：在 [src/lazybull/backtest/engine.py](src/lazybull/backtest/engine.py) 中，`最终` 字段不再重复展开 `信号门控 x 质量 x ECT x 市场层` 乘法明细，改为简洁状态标签：正常路径输出 `最终=xx.x%[入队/不入队]`，门控阻断路径输出 `最终=0.0%[门控阻断, 不入队]`，减少批量回测时单行日志长度。

### 测试

- 更新 [tests/test_ml_backtest_engine.py](tests/test_ml_backtest_engine.py)，同步修正调仓决策摘要 `最终` 段文案断言。

## [0.72.4] - 2026-05-29

### 优化

- **回测信号与交易摘要统一改为卖在前、买在后**：在 [src/lazybull/backtest/engine.py](src/lazybull/backtest/engine.py) 中，日终 `交易:` 摘要现在先输出卖出再输出买入；`信号:` 单行也调整为先展示卖出信号、再展示买入信号，便于按“先释放仓位、后补入仓位”的顺序阅读。

### 测试

- 更新 [tests/test_backtest_daily_progress_log.py](tests/test_backtest_daily_progress_log.py)，同步修正 `交易:` 与 `信号:` 的文本顺序断言，以及日终输出顺序断言。

## [0.72.3] - 2026-05-29

### 优化

- **回测信号摘要新增延续数量并调整输出顺序**：在 [src/lazybull/backtest/engine.py](src/lazybull/backtest/engine.py) 中，`信号:` 单行现在会额外汇总当日 `盈利延续` 的数量，即使当天没有新的买卖信号也会输出 `延续[N]`；同时其输出位置从交易摘要之前调整到 `交易: 买... / 卖...` 之后，更便于先看实际成交、再看当日新信号。

### 测试

- 更新 [tests/test_backtest_daily_progress_log.py](tests/test_backtest_daily_progress_log.py)，新增覆盖“仅有延续时仍输出信号摘要”的回归断言，并将信号摘要顺序断言调整为位于交易摘要之后。

## [0.72.2] - 2026-05-29

### 优化

- **回测新增 T 日信号数量摘要日志**：在 [src/lazybull/backtest/engine.py](src/lazybull/backtest/engine.py) 中，日终摘要现在会在调仓决策摘要之后、实际成交之前，额外输出一行简洁的 `信号:` 统计；按当日新生成的 `pending_signals`、`pending_condition_sells`、`pending_stop_loss_sells` 汇总买卖信号数量，例如调仓买入、补槽买入、持有期卖出、亏损换出、止损卖出等，便于区分“T 日信号”和“T+1 实际成交”。

### 测试

- 更新 [tests/test_backtest_daily_progress_log.py](tests/test_backtest_daily_progress_log.py)，新增覆盖“日级信号数量摘要格式”和“输出顺序位于调仓摘要之后、交易摘要之前”的回归断言。

## [0.72.1] - 2026-05-29

### 修复

- **回测盈利延续/持有期卖出后补回对应买入槽位**：在 [src/lazybull/backtest/engine.py](src/lazybull/backtest/engine.py) 中，`holding_period` 路径现会在 T0 生成卖出信号的同时，基于当日候选排序写入下一交易日的待买计划；T+1 先按实际卖出结果释放空槽，再按 T0 候选顺位补买，不再出现“盈利延续失败持续卖出但没有对应补买”的空仓漂移。
- **回测部分补买新增显式目标持仓数**：同文件的 `pending_signals` 现持久化 `desired_position_count`，用于在 T+1 根据实际卖出成功数量收缩可开仓槽位，避免部分补买路径把 `slot_weights` 数量误当成组合总目标持仓数。

### 测试

- 更新 [tests/test_backtest_t1.py](tests/test_backtest_t1.py)，新增覆盖“盈利延续未通过后，下一交易日先卖再补买”的回归断言。

## [0.72.0] - 2026-05-29

### 修复

- **纸面交易买入改为 T0 计划、T+1 执行并支持同日顺延**：在 [src/lazybull/paper/runner.py](src/lazybull/paper/runner.py) 与 [src/lazybull/paper/runtime.py](src/lazybull/paper/runtime.py) 中，T0 现会持久化 ML ranked_candidates；T+1 执行原计划买单失败后，会先在同一个交易日按 T0 候选优先级立即顺延补位，只有同日候选耗尽时才继续落入跨日 pending_buys 队列。
- **纸面交易卖出失败不再继续超配买入**：在 [src/lazybull/paper/broker.py](src/lazybull/paper/broker.py) 与 [src/lazybull/paper/models.py](src/lazybull/paper/models.py) 中，为交易指令补充 T0 目标持仓数元数据；T+1 买入阶段现按实际已释放空槽控制新开仓，若卖出因跌停/停牌等失败，当日不会继续新增超额仓位。
- **补位优先复用持久化 T0 候选**：在 [src/lazybull/paper/runner.py](src/lazybull/paper/runner.py) 中，`_execute_pending_buys()` 会优先读取已保存的 ranked_candidates 及原始 signal_date，对齐回测“先用 T0 候选、再跨日兜底”的顺序语义。

### 测试

- 更新 [tests/test_buy_replacement.py](tests/test_buy_replacement.py)，新增覆盖“卖出失败不再新开仓”与“补位优先读取 T0 持久化候选”的回归断言。
- 更新 [tests/test_paper_trade_runtime.py](tests/test_paper_trade_runtime.py)，新增覆盖“runtime 在同一 T1 内立即处理失败买单顺延补位”的回归断言。

## [0.71.79] - 2026-05-29

### 修复

- **回测买入改为 T0 计划、T+1 按优先级顺位执行**：在 [src/lazybull/backtest/engine.py](src/lazybull/backtest/engine.py) 中，`pending_signals` 现会保留 T0 的 ML 买入候选优先级和槽位权重；T+1 执行时若原计划股票不可买或未成交，会在同日按候选顺序继续顺延，只有同日候选耗尽时才进入后续跨日补齐。
- **卖出失败不再继续超配买入**：同文件中，T+1 买入数量现按实际空槽数收缩；若卖出因跌停或停牌进入延迟卖出队列，当日不会继续按旧计划新增仓位。

### 测试

- 更新 [tests/test_position_completion.py](tests/test_position_completion.py)，将补齐回归口径调整为“同日顺延优先，跨日补齐兜底”。
- 更新 [tests/test_backtest_t1.py](tests/test_backtest_t1.py)，新增覆盖“T0 买入候选在 T1 同日顺延”与“卖出失败不超配买入”的回归断言。
...
- **回测持有期/盈利延续卖出改为 T0 信号、T+1 执行**：在 [src/lazybull/backtest/engine.py](src/lazybull/backtest/engine.py) 中，将“持有期到期、盈利延续未通过、盈利延续到期”三条预定卖出路径统一改为在触发日写入 `pending_condition_sells`，由下一交易日执行，避免继续沿用当日直卖口径。
- **回测交易规则说明同步修正**：同文件初始化日志中的交易规则描述改为“满持有期后 T0 生成卖出信号，下一交易日按卖出时机成交”，与当前回测实现保持一致。

### 测试

- 更新 [tests/test_backtest_t1.py](tests/test_backtest_t1.py)，新增覆盖“持有期到期、盈利延续未通过、盈利延续到期”三条 T0->T1 卖出路径的回归断言，并校正盈利延续摘要日志断言。
...
- **3.5LCD 图表模块中文文本恢复**：恢复 [scripts/respi/lcd35/charting.py](scripts/respi/lcd35/charting.py) 中被编码破坏的中文注释、docstring 和界面文案，保持现有拆分逻辑不变

## [0.71.51] - 2026-05-12

### 修复

- **3.5LCD 拆分后共享命名空间恢复**：主入口恢复按源码顺序 `exec` 到同一 `globals()`，并为各组件名注册快照模块，既保留拆分后的静态可读性，也恢复测试与运行时对共享命名空间和环境变量重读的预期
- **3.5LCD 组件显式导入补齐**：补齐行业页、图表页、系统 IO 和渲染页之间的下划线辅助函数导入，消除拆分后遗漏的运行时 `NameError`
- **3.5LCD 行业统计口径修正**：行业页按已知行业统计 L1/L2/L3 数量，避免未知行业污染计数

### 测试

- 更新 `tests/test_respi_35lcd_disp.py` 定向回归，覆盖旧入口可加载、环境变量读取、网络上下文清理、行业面板统计与渲染前缀

## [0.71.50] - 2026-05-12

### 重构

- **3.5LCD 主入口重命名**：新增 `scripts/respi/lcd35_display.py` 作为主入口，`scripts/respi/3.5LCD_disp.py` 改为仅负责兼容跳转的历史壳文件
- **lcd35 组件去数字化命名**：组件文件统一改为 `core.py`、`industry.py`、`charting.py`、`system_io.py`、`data_pipeline.py` 等更直观的命名
- **运行时职责继续细化**：原运行时组件进一步拆成 `state.py`、`rendering.py`、`app.py`，将状态定义、绘制逻辑和线程/入口逻辑分离

### 测试

- 更新 `tests/test_respi_35lcd_disp.py` 默认加载新主入口，并增加旧入口兼容可加载校验

## [0.71.49] - 2026-05-12

### 重构

- **3.5LCD 显示脚本按功能拆分**：原 `scripts/respi/3.5LCD_disp.py` 拆分为 `scripts/respi/lcd35/` 下的核心、行业统计、图表工具、系统 IO、数据链路与运行时组件，原文件保留为兼容加载入口
- 在不改变用户可见功能与显示效果的前提下，降低单文件复杂度，便于后续功能叠加与阅读维护

### 优化

- **3.5LCD 运行内存优化**：快照缓存复制改为只克隆必要字段，减少递归 `deepcopy` 带来的额外内存峰值
- framebuffer 输出、启动页、错误页与主渲染页写屏后显式释放 PIL 图像对象，缓解树莓派小内存场景下的瞬时占用
...
- **3.5LCD 盘后重启空面板快速重试修复** ([scripts/respi/3.5LCD_disp.py](scripts/respi/3.5LCD_disp.py)):
  - 新增实时抓取失败快速重试机制：当本轮实时刷新后仍无有效持仓快照时，不再等待常规 180/600 秒，改为 15 秒快速重试
  - 解决盘后重启首轮抓数超时后摘要/排行/行业长时间为空的问题
  - 新增诊断日志：`实时抓取未命中有效快照，将于15s后快速重试`
...
- **修复树莓派启动后所有数据为空（`from paper_trade import` 路径问题）** ([scripts/respi/3.5LCD_disp.py](scripts/respi/3.5LCD_disp.py)):
  - `_build_realtime_portfolio_summary` 用 `__file__` 推导 `scripts/` 绝对路径并插入 `sys.path`
  - 修复非项目根目录启动时 `ImportError` 被静默吞掉导致 `summary=None`、所有指标空白的问题
...
- **修复批量纸面交易脚本的 PowerShell 未认可动词告警** ([scripts/batch_paper_trade.ps1](scripts/batch_paper_trade.ps1)):
  - 将函数名 `Parse-DateText` 重命名为 `Get-DateFromText`
  - 同步更新调用点，消除 PSScriptAnalyzer 的 `UseApprovedVerbs` 告警

## [0.70.10] - 2026-05-05

### 修复

- **修复补位真实执行路径漏掉最小买入后市值阈值的问题** ([src/lazybull/paper/runner.py](src/lazybull/paper/runner.py)):
  - `T1` 阶段补位实际执行走 `PaperTradingRunner._execute_pending_buys()`，此前未做 `min_buy_value_ratio` 阈值拦截
  - 现已在候选股可买股数计算后增加阈值校验：`买入后市值 >= (总资产 / top_n) * min_buy_value_ratio`
  - 低于阈值时不下单，保留为补位延迟，避免出现 100 股几百元碎仓

### 测试

- **新增补位真实执行路径回归测试** ([tests/test_buy_replacement.py](tests/test_buy_replacement.py)):
  - `test_execute_pending_buys_skip_tiny_buy_value_by_ratio`
...
- **纸面交易新增最小买入后市值阈值，避免出现几百元碎仓买入** ([src/lazybull/paper/broker.py](src/lazybull/paper/broker.py), [src/lazybull/common/trading_config.py](src/lazybull/common/trading_config.py), [src/lazybull/paper/storage.py](src/lazybull/paper/storage.py), [scripts/paper_trade.py](scripts/paper_trade.py)):
  - 新增配置 `min_buy_value_ratio`（默认 `0.2`，设为 `0` 可关闭）
  - 阈值定义：`最小买入后市值 = (总资产 / top_n) * min_buy_value_ratio`
  - 在三条路径统一生效：T0 生成订单买入、T1 指令买入、补位重试买入
  - 当买入后市值低于阈值时跳过买入并保留现金，不再生成过小持仓

### 测试

- **新增最小买入后市值阈值回归测试** ([tests/test_paper_trading.py](tests/test_paper_trading.py)):
  - `test_broker_execute_instructions_skip_tiny_buy_value_by_ratio`
  - `test_broker_retry_pending_buys_keeps_order_when_value_too_small`

## [0.70.8] - 2026-05-05

### 修复

- **补位延迟日志简化为单句摘要，移除示例明细** ([src/lazybull/paper/runner.py](src/lazybull/paper/runner.py)):
  - 保留核心信息：交易日、槽位、候选池数量、原因汇总、下次重试
  - 移除冗长的 `示例[...]` 内容，避免日志过长影响阅读

### 测试

- **纸面交易相关回归通过**:
  - `tests/test_paper_trading.py`
  - `tests/test_paper_trade_runtime.py`

## [0.70.5] - 2026-05-05

### 修复

- **纸面持仓表“剩余D”纳入延期持有窗口计算** ([src/lazybull/paper/broker.py](src/lazybull/paper/broker.py)):
  - 旧逻辑仅按 `rebalance_freq` 计算 `剩余D`
  - 新逻辑在 `enable_profit_based_holding=true` 且 `profit_extension_mode!=disabled` 时，按
    `rebalance_freq + profit_extension_days` 作为可持有上限计算剩余
  - 示例口径：`剩余D = max(0, (rebalance_freq + extension_days) - 持有交易日)`

### 测试

- **新增延期持有剩余天数回归测试** ([tests/test_paper_trading.py](tests/test_paper_trading.py)):
  - `test_broker_positions_remaining_includes_extension_days`
  - 验证开启盈利延续时，`剩余D` 会包含 `profit_extension_days`

## [0.70.7] - 2026-05-05

### 修复

- **补位延迟日志增加原因统计，明确区分现金不足与其他拒绝原因** ([src/lazybull/paper/runner.py](src/lazybull/paper/runner.py)):
  - `_execute_pending_buys()` 在槽位补位失败时输出原因聚合与示例股票
  - 新增 `_analyze_pending_buy_shares_backtest_style()`，对“目标金额不足一手 / 现金不足 / 手续费不足 / 缩量后不足一手”等资金约束场景给出明确判定
  - 修复效果：日志中的“未找到可买入股票”从笼统描述升级为可解释的分项原因，便于快速定位是否由现金导致

### 测试

- **补位执行与运行时相关回归通过**:
  - `tests/test_buy_replacement.py`
  - `tests/test_paper_trade_runtime.py`
...
- **纸面补位队列改为保留“未成交槽位”原始权重** ([src/lazybull/paper/runtime.py](src/lazybull/paper/runtime.py)):
  - `_handle_failed_buys()` 不再在 T1 预先重算替代股票并重置权重
  - 改为直接保存失败买入槽位（`ts_code` + `target_weight`），下一交易日再按 D-1 数据重算候选
  - 修复效果：避免补位目标被重算为等权，执行口径与回测 `unfilled_slot_weights` 一致

- **纸面补位买入金额改为回测同款“组合价值×槽位权重”** ([src/lazybull/paper/runner.py](src/lazybull/paper/runner.py)):
  - 新增 `_estimate_pending_buy_shares_backtest_style()`，补位目标金额按 `current_total_value * target_weight`
  - 现金不足时按剩余现金缩量买入，而非按“补位均分现金+保留比例”估算
  - 修复效果：补位仓位规模与回测一致，不再出现补位金额系统性偏小

- **纸面补位候选池重算增加交易日股票池约束并统一可交易性判定** ([src/lazybull/paper/runner.py](src/lazybull/paper/runner.py), [src/lazybull/paper/runtime.py](src/lazybull/paper/runtime.py)):
  - `_execute_pending_buys()` 在 D-1 候选重算时使用 `universe/exclude_st/min_list_days` 配置构建股票池
  - 补位可交易性检查统一走 `common.trade_status.is_tradeable()`
  - 修复效果：候选来源与交易状态判定更贴近回测补齐路径

### 测试

- **更新补位执行对齐测试** ([tests/test_buy_replacement.py](tests/test_buy_replacement.py)):
  - `test_execute_pending_buys_uses_limited_candidate_pool_by_previous_day` 增加回测口径股数断言（`2500` 股）
  - 显式注入 D-1/当日行情与股票池，稳定覆盖补位执行路径
- **新增补位队列权重保留测试** ([tests/test_paper_trade_runtime.py](tests/test_paper_trade_runtime.py)):
  - `test_handle_failed_buys_preserves_original_slot_weight`

## [0.70.5] - 2026-05-05

### 修复

- **纸面补位执行改为回测同款"有限候选池逐槽位匹配"** ([src/lazybull/paper/runner.py](src/lazybull/paper/runner.py)):
  - `_execute_pending_buys()` 基于上一交易日重算候选池，候选数限制为"槽位数×2"，再逐槽匹配
  - 排除已持仓、已买入和不可交易的候选，保持与回测 `_process_position_completion` 逻辑一致
  - 候选重算失败时保留回退路径（使用队列代码模式），保证异常场景可用性
  - 新增 `_get_prev_trade_date()` 辅助方法
  - 修复效果：减少"纸面一次性补满所有槽位 vs 回测逐日有限补齐"导致的持仓数量大幅分叉

- **纸面 T0 在 `holding_bonus=false` 时完全排除已持仓股票** ([src/lazybull/paper/runner.py](src/lazybull/paper/runner.py)):
  - `_generate_ranked_with_lot_constraint()` `holding_bonus_enabled=False` 时先移除 `existing_positions`
  - 修复效果：不再给已持仓股票生成补差买单

### 测试

- **新增补位算法对齐回归测试** ([tests/test_buy_replacement.py](tests/test_buy_replacement.py)):
  - `test_execute_pending_buys_uses_limited_candidate_pool_by_previous_day`：验证两槽位基于前一日候选池各买一只不重复股票
- **新增 `holding_bonus=false` 排除持仓回归测试** ([tests/test_equal_weight_lot_constraint.py](tests/test_equal_weight_lot_constraint.py)):
  - `test_equal_weight_lot_constraint_excludes_existing_positions_when_holding_bonus_disabled`

## [0.70.4] - 2026-05-05

### 修复

- **纸面持仓表“持有天数”改为交易日口径，并新增“持有剩余”列** ([src/lazybull/paper/broker.py](src/lazybull/paper/broker.py)):
  - `get_positions_detail()` 优先按交易日历计算持有天数（不含买入当日），交易日历缺失时回退自然日
  - 按 `rebalance_freq` 实时计算每只持仓“持有剩余”交易日：`max(0, rebalance_freq - holding_days)`
  - 持仓汇总表头与行渲染同步扩展为 11 列，新增“持有交易日/持有剩余”展示

### 测试

- **更新纸面持仓明细相关断言** ([tests/test_paper_trading.py](tests/test_paper_trading.py)):
  - 持有天数断言改为交易日口径
  - 新增“持有剩余”字段断言
...
- **批量纸面交易脚本输出文案恢复中文** ([scripts/batch_paper_trade.ps1](scripts/batch_paper_trade.ps1)):
  - 脚本注释、提示日志、错误信息统一改为中文
  - 保留已验证稳定的 `trade-date=next` 推进逻辑与 `py -c` 引号安全写法

## [0.70.2] - 2026-05-05

### 修复

- **批量纸面交易改为 next 交易日推进，避免仅按周末过滤导致的非交易日误判** ([scripts/batch_paper_trade.ps1](scripts/batch_paper_trade.ps1)):
  - 首次执行使用 `--trade-date <start_date>`，后续统一使用 `--trade-date next`
  - 通过 `data/clean/trade_cal.parquet` 解析下一开市日，确保停止边界不超过 `end_date`
  - 保留脚本内配置区输入方式（`start_date/end_date/reset_t0_before_run`）

## [0.70.1] - 2026-05-05

### 修复

- **简化纸面交易批量脚本输入方式** ([scripts/batch_paper_trade.ps1](scripts/batch_paper_trade.ps1)):
  - 移除命令行参数 `-StartDate/-EndDate/-ResetT0`
  - 改为脚本内“参数配置区”直接填写 `$start_date`、`$end_date`、`$reset_t0_before_run`
  - 保留原有日期校验、周末跳过、进度/ETA、失败统计与退出码行为

## [0.70.0] - 2026-05-05

### 新增

- **新增纸面交易批量执行脚本** ([scripts/batch_paper_trade.ps1](scripts/batch_paper_trade.ps1)):
  - 支持 `-ResetT0` 开关：控制开始执行前是否先调用 `paper_trade.py adjust reset-t0`
  - 支持 `-StartDate` 与 `-EndDate` 参数：按日期区间逐日批量执行 `paper_trade.py run --trade-date`
  - 默认仅执行周一到周五，并输出进度、ETA、失败统计与退出码（有失败时返回非 0）

## [0.69.18] - 2026-05-05

### 修复

- **纸面 T0 在 `holding_bonus=false` 时完全排除已持仓股票，避免生成补差买单** ([src/lazybull/paper/runner.py](src/lazybull/paper/runner.py)):
  - `_generate_ranked_with_lot_constraint()` 在候选筛选阶段新增分流：当 `holding_bonus_enabled=False` 时先移除 `existing_positions`
  - 一手约束循环中仅在 `holding_bonus_enabled=True` 时才允许已持仓股票直接入选
  - 修复效果：调仓日不再出现“先卖后小额买回同一只已持仓股票”的补差指令

### 测试

- **新增回归测试** ([tests/test_equal_weight_lot_constraint.py](tests/test_equal_weight_lot_constraint.py)):
  - `test_equal_weight_lot_constraint_excludes_existing_positions_when_holding_bonus_disabled`
  - 验证 `holding_bonus=false` 时，已持仓股票不会出现在 T0 目标中

## [0.69.17] - 2026-05-05

### 修复

- **修复 ranked_candidates 因保存条件过严而漏落盘，导致后续持仓评分 `ml_raw=0`** ([src/lazybull/paper/runtime.py](src/lazybull/paper/runtime.py)):
  - 持久化条件由“`t0_status=success` 且 `_last_signal_date` 与当日严格匹配”调整为“`t0_status=success` 且候选池非空”
  - 避免 `_last_signal_date` 类型/格式差异（例如非 `pd.Timestamp`）导致候选池文件未保存
  - 直接修复现象：`data/paper/state/ranked_candidates.json` 缺失时，次日 `strength` 评分会大面积 `raw=0.000`

### 测试

- **回归验证通过**:
  - `tests/test_paper_trade_runtime.py`
  - `tests/test_paper_holding_period_alignment.py`

## [0.69.16] - 2026-05-05

### 修复

- **修复非调仓日 ranked_candidates 被补位流程覆盖，导致持仓 strength 评分失真** ([src/lazybull/paper/runtime.py](src/lazybull/paper/runtime.py)):
  - `execute_trade_workflow()` 仅在 `t0_status=success` 且 `signal._last_signal_date == corrected_date` 时保存 ranked_candidates
  - 避免非调仓日（如仅执行 T1 + 补位）把“补位候选池”误写成下一日持仓评分基准，导致 `ml_raw=0` 大面积出现

- **修复 strength 按日评估未确保当日特征导致分项退化为 0.5** ([src/lazybull/paper/runner.py](src/lazybull/paper/runner.py)):
  - 在 `evaluate_holding_period_actions()` 的 `profit_extension_mode=strength` 路径中，先执行 `ensure_features_for_date(trade_date)`
  - 缺失当日 `cs_infer` 时不再静默退化，优先补齐后再评分，减少 `mom/tech/fund=0.50` 的系统性偏差

### 测试

- **新增 runtime ranked_candidates 持久化边界测试** ([tests/test_paper_trade_runtime.py](tests/test_paper_trade_runtime.py)):
  - 覆盖“非调仓日不保存 ranked_candidates”
  - 覆盖“T0 成功且信号日期匹配时保存 ranked_candidates”

- **新增 strength 评估前特征补齐测试** ([tests/test_paper_holding_period_alignment.py](tests/test_paper_holding_period_alignment.py)):
  - 覆盖 `evaluate_holding_period_actions()` 在 `strength` 模式下会先 ensure 当日特征

## [0.69.15] - 2026-05-05

### 修复

- **纸面 T0 买入指令顺序改为按目标顺序生成，消除 set 无序导致的现金受限漂移** ([src/lazybull/paper/runner.py](src/lazybull/paper/runner.py)):
  - `_generate_instructions()` 不再使用 `set(target_weights.keys())` 遍历目标股票
  - 改为按 `targets` 原始顺序生成买入/加仓指令，保持与信号排名顺序一致
  - 在现金不足场景下，成交先后将可复现，减少纸面与回测在同日因执行顺序差异造成的持仓数量/成分偏移

### 测试

- **新增 T0 指令顺序回归测试** ([tests/test_ensure_and_t0_printing.py](tests/test_ensure_and_t0_printing.py)):
  - 覆盖 `_generate_instructions()` 输出买单顺序与 `targets` 输入顺序一致

## [0.69.14] - 2026-05-05

### 修复

- **纸面交易 early_exit 触发区间对齐回测，避免到期日抢跑卖出** ([src/lazybull/paper/runner.py](src/lazybull/paper/runner.py)):
  - `evaluate_early_exit()` 新增持有期边界：仅在 `holding_days < rebalance_freq` 时评估亏损提前换出
  - 到达/超过持有期后，提前换出让位于“持有期到期 + 盈利延续”路径，避免同一交易日先入 `pending_sells` 再做延续判定导致的执行分叉
  - 预期效果：减少回测与纸面在持有期到期窗口（如你反馈的后续调仓日）出现的大规模卖出差异

- **runtime 恢复 ranked_candidates 增加格式校验，避免异常返回中断工作流** ([src/lazybull/paper/runtime.py](src/lazybull/paper/runtime.py)):
  - `execute_trade_workflow()` 在恢复 `ranked_candidates` 时，仅对 `(candidates, signal_date)` 二元组执行解包
  - 当返回值为异常结构时改为 warning 并跳过恢复，避免 `ValueError` 中断当日执行

### 测试

- **新增 early_exit 持有期边界回归测试** ([tests/test_paper_holding_period_alignment.py](tests/test_paper_holding_period_alignment.py)):
  - 覆盖“到达持有期后即使亏损，也不应再触发 early_exit”

## [0.69.13] - 2026-05-05

### 修复

- **ensure 构建 cs_infer 口径与 build_clean_features 对齐（除标签过滤外）** ([src/lazybull/features/ensure.py](src/lazybull/features/ensure.py)):
  - 历史窗口从 1 个月调整为与离线构建一致的 `7` 个月 warmup，并保持向后扩展 `1` 个月的数据加载口径
  - `ensure_features_for_date()` 新增 `builder.precompute_daily_adj(...)` 预计算路径，与离线构建的特征预热流程保持一致
  - 历史 clean 分区补齐窗口扩展为 warmup 口径（最多检查最近 180 个交易日），降低历史缺口导致的技术因子偏移
  - moneyflow 缺失处理与离线构建对齐为“告警但不中断”，避免 ensure 与离线流程的异常语义分叉
  - 目标效果：同日期同模型下，纸面 `cs_infer` 与回测 `cs_train` 的输入口径更一致，减少 TopN 候选分叉

### 测试

- **新增 ensure 构建窗口与预计算路径回归测试** ([tests/test_ensure_and_t0_printing.py](tests/test_ensure_and_t0_printing.py)):
  - 覆盖 `7个月 warmup + 1个月扩展` 的 clean 数据加载窗口
  - 覆盖 `builder.precompute_daily_adj(...)` 调用
  - 覆盖 moneyflow 缺失时 ensure 不中断的兼容路径

## [0.69.12] - 2026-05-04

### 修复

- **纸面交易跨运行持久化 ranked_candidates 供 T1 持仓评分使用（根本修复）** ([src/lazybull/paper/storage.py](src/lazybull/paper/storage.py), [src/lazybull/paper/runtime.py](src/lazybull/paper/runtime.py)):
  - 根本原因：纸面交易每次 `python scripts/paper_trade.py run` 都会创建全新的 signal 实例，导致前一个 T0 的 `_last_ranked_candidates` 丢失
  - 结果：T1 执行 `_score_holding_strength` 时无法获取真实 ml_raw_score，导致 ml 维度降级计算（用 acceleration + alpha_industry 近似），导致 ml 分数显著偏低
  - 修复方案：在 PaperStorage 中新增 `save_ranked_candidates()` 和 `load_ranked_candidates()` 方法，持久化存储到 `data/paper/state/ranked_candidates.json`
  - 执行流程改进：T1 前恢复前一个 T0 的 ranked_candidates，T0 后保存本轮的 ranked_candidates 供下一个 T1 使用
  - 验证结果：纸面 ml 维度分数和 raw_score 恢复与回测一致

## [0.69.11] - 2026-05-04

### 修复

- **MLSignal.generate_ranked 缓存 `_last_ranked_candidates` 供纸面持仓强势度评分使用** ([src/lazybull/signals/ml_signal.py](src/lazybull/signals/ml_signal.py)):
  - 纸面 `_score_holding_strength` 通过 `signal._last_ranked_candidates` 查询持仓股 ml_raw_score
  - 原先 MLSignal 未设置该属性，导致所有持仓的 `raw=0.000`、ml 维度降级为 acceleration 估算值
  - 与回测对齐：回测 engine 在 `_generate_signal` 末尾缓存完整候选列表，持仓股 raw 有真实预测分
  - 修复后 strength 评分从被系统性低估恢复正确，持仓延续决策更准确

- **pending_sells 重试跳过创建日当天或之后新买入的仓位（BUG修复）** ([src/lazybull/paper/broker.py](src/lazybull/paper/broker.py)):
  - 场景：T0 亏损换出将 A 股加入 pending_sells(create_date=D)，T1 同日先通过持有期路径卖出 A 的大仓，再按新信号买入 A 的小仓(buy_date=D)
  - 旧代码：下一日重试 pending_sells 时，发现 A 有持仓即卖出，错误卖掉了新建仓位
  - 修复：在 `retry_pending_sells` 中检查 `pos.buy_date >= ps.create_date`，满足则说明是新仓，跳过旧卖单
  - 此 BUG 导致纸面额外减仓，累计影响不可忽视
...
- **持仓保留奖励命中后将持有期锚点重置到 T+1，避免延续后提前到期卖出** ([src/lazybull/paper/runner.py](src/lazybull/paper/runner.py), [tests/test_paper_holding_period_alignment.py](tests/test_paper_holding_period_alignment.py)):
  - 在 `_generate_signals()` 中，当 `holding_bonus_enabled` 且留仓命中时，新增锚点重置流程
  - 新增 `_reset_holding_anchor_for_kept_positions()`，将留仓 `buy_date` 从旧值推进到 T+1，并持久化账户状态
  - 对齐回测 `_extend_holding_period()` 语义，修复“延续持有但持有期起点未后移”导致的持有窗口偏短问题

### 测试

- **新增持仓保留奖励锚点重置回归测试** ([tests/test_paper_holding_period_alignment.py](tests/test_paper_holding_period_alignment.py)):
  - 覆盖“留仓命中后 buy_date 重置到 T+1”以及账户状态持久化

## [0.69.9] - 2026-05-04

### 修复

- **纸面交易卖出主路径对齐回测，并统一盈亏口径为后复权绩效价** ([src/lazybull/paper/runner.py](src/lazybull/paper/runner.py), [src/lazybull/paper/broker.py](src/lazybull/paper/broker.py), [src/lazybull/paper/account.py](src/lazybull/paper/account.py), [src/lazybull/paper/models.py](src/lazybull/paper/models.py), [src/lazybull/paper/storage.py](src/lazybull/paper/storage.py)):
  - `PaperTradingRunner._generate_instructions()` 调整为仅生成买入/加仓，目标权重减仓/清仓不再直接下卖单
  - 卖出统一走“持有期到期/条件触发（盈利延续、提前换出等）”路径，避免与回测实现分叉
  - `evaluate_profit_extension()` / `evaluate_holding_period_actions()` / `evaluate_early_exit()` 统一以后复权绩效价计算 `profit_rate`
  - 买入成交时记录 `buy_pnl_price`，并在账户状态序列化中持久化，保证跨日盈亏计算口径稳定
  - T0 目标详情展示同步改为“保留（持有期/条件驱动卖出）”，避免展示与实际执行冲突

### 测试

- **新增回测-纸面同窗口决策对齐测试与后复权口径测试** ([tests/test_paper_holding_period_alignment.py](tests/test_paper_holding_period_alignment.py)):
  - 覆盖“目标减仓不再生成直接卖出指令”
  - 覆盖“盈利延续判定使用后复权绩效价”
  - 覆盖“同区间下回测与纸面对盈利延续决策一致”

## [0.69.8] - 2026-05-04

### 修复

- **纸面交易按日执行“持有期到期/盈利延续”评估，避免仅调仓日触发造成与回测偏离** ([src/lazybull/paper/runner.py](src/lazybull/paper/runner.py), [src/lazybull/paper/runtime.py](src/lazybull/paper/runtime.py)):
  - 新增 `PaperTradingRunner.evaluate_holding_period_actions()`，按交易日评估到期卖出与盈利延续（`pnl/strength/disabled`）
  - 将该评估接入 `runtime._execute_t1_if_pending()`：即使当日无调仓指令，也会执行持有期到期卖出
  - 解决纸面交易中“盈利延续几乎仅在调仓日少量触发”的口径偏差，缩小与回测实现差异

### 测试

- **新增持有期对齐回归测试** ([tests/test_paper_holding_period_alignment.py](tests/test_paper_holding_period_alignment.py)):
  - 覆盖“非调仓日也可触发 strength 延续”
  - 覆盖“无调仓指令时 T1 仍执行持有期到期卖出”
...
- **修复纸面交易 T0 路径 `protected_stocks` 未定义导致执行失败** ([src/lazybull/paper/runner.py](src/lazybull/paper/runner.py)):
  - 为 `_generate_signals()` 增加 `protected_stocks` 可选参数，并在 `run_t0()` 中透传
  - 修复日志报错 `T0 执行失败: name 'protected_stocks' is not defined`
  - 保持盈利延续保护展示链路可用，不影响最终指令生成

## [0.69.6] - 2026-05-04

### 修复

- **钉钉机器人交易结果新增“盈利延续保护”紧凑展示** ([src/lazybull/paper/runtime.py](src/lazybull/paper/runtime.py), [src/lazybull/paper/reporting.py](src/lazybull/paper/reporting.py), [tests/test_paper_trade_runtime.py](tests/test_paper_trade_runtime.py)):
  - 共享运行时结果新增 `protected_stocks`，将 T0 阶段命中的盈利延续保护名单透传到展示层
  - `format_trade_result()` 在钉钉 Markdown 中新增紧凑的 `--- 盈利延续保护 ---` 区块，列出被保护保留的股票
  - 为适应钉钉消息长度限制，最多展示前 8 只，超出时追加“其余 N 只已省略”

## [0.69.5] - 2026-05-04

### 修复

- **T0 目标详情展示与盈利延续保护后的最终指令对齐** ([src/lazybull/paper/runner.py](src/lazybull/paper/runner.py), [tests/test_paper_trading.py](tests/test_paper_trading.py)):
  - `T0 建仓目标详情` 现在会识别 `protected_stocks`，对被盈利延续保护的持仓显示为 `保留`，不再误报为 `清仓/减仓`
  - 打印目标股数时改为复用与最终指令一致的总资产/现金保留比例口径，降低展示与真实指令不一致的概率
  - 修复“无新目标但有受保护持仓”时日志提前返回的问题
  - 新增回归测试，约束受保护股票在详情表中显示为 `保留`

## [0.69.4] - 2026-05-04

### 修复

- **修复纸面交易 strength 路径读取特征时报 AttributeError** ([src/lazybull/paper/runner.py](src/lazybull/paper/runner.py), [tests/test_paper_trading.py](tests/test_paper_trading.py)):
  - 将纸面交易运行器中两处不存在的 `Storage.load_features_by_date(...)` 调用改为现有接口 `Storage.load_cs_train_day(..., subdir="cs_infer")`
  - 修复在 T0 阶段“计算盈利延续保护”时触发 `AttributeError: 'Storage' object has no attribute 'load_features_by_date'` 的问题
  - 新增回归测试，约束 strength 评分路径使用 `load_cs_train_day`，避免同类接口名漂移再次引发运行时异常

## [0.69.3] - 2026-05-04

### 修复

- **纸面交易中盈利延续 strength 命中保护时改为 warning 日志，提升可见性** ([src/lazybull/paper/runner.py](src/lazybull/paper/runner.py), [tests/test_paper_trading.py](tests/test_paper_trading.py)):
  - `evaluate_profit_extension()` 在 `profit_extension_mode=strength` 且强势度达到阈值时，日志级别由 `info` 提升为 `warning`
  - 日志文案显式展示 `强势度 >= 阈值`，更容易在纸面交易执行输出中识别“因盈利延续而未卖出”的持仓
  - 新增回归测试，约束该 warning 日志持续存在

## [0.69.2] - 2026-05-04

### 修复

- **batch walk-forward 支持扫描 MA250 硬条件阈值与仓位，并在对比 xlsx 中保留参数列** ([scripts/batch_walk_forward.ps1](scripts/batch_walk_forward.ps1), [scripts/compare_walk_forward.py](scripts/compare_walk_forward.py), [scripts/walk_forward.py](scripts/walk_forward.py)):
  - 将 `market_regime_ma250_threshold` 与 `market_regime_ma250_exposure` 从单值参数改为列表扫描参数，纳入批量任务笛卡尔积
  - 在批量循环中逐组合透传 `--market-regime-ma250-threshold/--market-regime-ma250-exposure`
  - 汇总链路沿用现有 summary/compare 参数列映射，生成的对比 xlsx 可直接看到两列参数值

## [0.69.1] - 2026-04-30

### 修复

- **compare 跨时间段稳定性汇总去重：同一参数组同一时间段仅保留最新 run** ([scripts/compare_walk_forward.py](scripts/compare_walk_forward.py)):
  - 修复 `wf_comparison_batches.xlsx` 中“时间段列表/运行ID列表”被同一时间段重复 run 放大（例如出现大量 `0101 | 0101 | ...`）的问题
  - 在稳定性聚合前，按 `参数组 + 时间段标签` 去重，并基于 `wf_run_id` 时间戳保留最新执行结果
  - 使 `时间段数` 回到预期口径（每个参数组按不同 `wf_period_configs` 统计）

### 测试

- **新增跨时间段稳定性去重回归测试** ([tests/test_ma250_observability.py](tests/test_ma250_observability.py)):
  - 覆盖同一批次同一时间段存在重复 run 时，仅保留最新 run 参与稳定性统计
...
- **树莓派 3.5 寸 LCD 行业页与顶部资源条样式进一步优化** ([scripts/respi/3.5LCD_disp.py](scripts/respi/3.5LCD_disp.py)):
  - 图表页/行业页轮播时长改为 `30秒 : 30秒`
  - 行业列表按收益（行业盈亏金额）从高到低排序，收益低的排在后面
  - 行业页首行（行业层级数量 + 正负收益统计）增加底色，和数据区区分更明显
  - 行业区改为轻量表格风格，并增加中间分栏加粗线
  - 行业名显示长度由 10 字提升到 12 字
  - 顶部 CPU/内存进度条去除边框，仅保留填充条与分区

### 测试

- **更新 3.5 寸 LCD 回归测试覆盖上述样式与排序调整** ([tests/test_respi_35lcd_disp.py](tests/test_respi_35lcd_disp.py)):
  - 轮播时长测试改为 30/30
  - 行业排序断言改为收益降序
  - 新增顶部资源条无边框断言
  - 新增行业表格分隔线绘制断言
...
- **树莓派 3.5 寸 LCD 行业页调整为 2 列 × 5 行紧凑展示，并优化首行统计** ([scripts/respi/3.5LCD_disp.py](scripts/respi/3.5LCD_disp.py)):
  - 行业页每页固定展示 10 个行业（2 列 × 5 行），字体较上一版增大
  - 每个行业行改为紧凑数值格式：`+正数/-负数/+贡献%`，减少中文占位
  - 数值颜色按正负实时渲染：正红、负绿、零白
  - 首行改为展示：`行业1/2/3:<L1>/<L2>/<L3>` 与 `正/负收益股票数量:+x/-y`，并对正负数分别着色
  - 行业分页逻辑与新布局同步（超出 10 个行业自动翻页覆盖全部行业）

### 测试

- **更新 3.5 寸 LCD 行业页渲染与聚合测试** ([tests/test_respi_35lcd_disp.py](tests/test_respi_35lcd_disp.py)):
  - 新增/调整 L1/L2/L3 行业数量字段断言
  - 更新紧凑格式文本断言（`+1/-1/+12.3%`）
  - 更新分页覆盖测试以匹配每页 10 行业的新布局
...
- **树莓派 3.5 寸 LCD 图表区新增“图表页/行业统计页”轮播与进度条** ([scripts/respi/3.5LCD_disp.py](scripts/respi/3.5LCD_disp.py)):
  - 图表区改为双页轮播：折线图页 40 秒，行业统计页 20 秒，仅切换图表区域内容
  - 图表区顶部新增 3 像素无边框进度条，实时显示当前页展示进度，进度走满自动切页
  - 新增行业统计页内容：
    - 持仓行业清单（按贡献度排序）
    - 各行业正/负收益股票数量
    - 各行业正收益 TOP1、负收益 TOP1
    - 各行业对总收益贡献比例
    - 全持仓正收益/负收益股票数量汇总
  - 行业归属严格使用申万行业映射，并按 `industry.shenwan_level` 动态选择 L1/L2/L3 主口径
  - 行业名称颜色按贡献收益实时变化：正收益红色，负收益绿色，零收益白色

### 测试

- **补充 3.5 寸 LCD 轮播与行业统计回归测试** ([tests/test_respi_35lcd_disp.py](tests/test_respi_35lcd_disp.py)):
  - 新增 40:20 轮播节奏测试
  - 新增行业名称颜色规则测试（正红/负绿/零白）
  - 新增行业统计聚合结果测试（行业盈亏计数、TOP1）
  - 新增图表区轮播渲染分支测试（图表页与行业页切换）

...
- **纸面暴露政策实盘模式（P1）**：`DailyExposureProvider` 新增 `coverage_mode`
  ...
  **继续使用**该折模型预测）与 `pinned_fold`（指定折名固定使用；构造期校验，无效折名报错）。
  ...
  （打分口径不变，面板状态可安全复用）；**指纹机器无关**（策略串 + 折后缀，**不含模型根绝对路径**，
  预热/状态文件可跨机器拷贝；**模型内容一致性由预热文件 `folds_digest`（折模型内容摘要）校验**）。
...
- **修复指纹跨机失配**：旧指纹含模型根**绝对路径** ⇒ 预热/状态文件拷到另一台机器必然失配；
  已改为机器无关（策略串 + 折后缀）。旧预热文件指纹作废，**需用新代码重新生成一次**。
- 修复面板延展的缓存判断（按“真实需求”而非“含缓冲需求”比较）——避免标签成熟收尾阶段逐日重载面板。
...
- **top_inst（龙虎榜机构席位）数据下载落地（探索期）**：`data/raw/top_inst/YYYY-12-31.parquet` 按 `trade_date` 年分区，
  2012-01-04 ~ 2026-09-22 共 15 个年分区、2,686,011 行（16 并发全量约 21.6 分钟）。
  抓取协议：逐交易日 + 单页 `limit=2000` + `offset` 翻页读到空（实测单日峰值 4,559 行 > 单页上限，不翻页必丢）；
  全字段去重。**采集工程教训**：v1 “全内存尾部统一落盘”在终端中断时 20 分钟数据全丢，
  v2 改为**按年分块写盘 + 断点续传**（脚本已归档 `data/reports/top_inst_20260923/`）。

### Verification

- 数据质量审计（全库 2,686,011 行）：行 =「席位 × side × reason」笛卡尔展开（键重复 13%~56%）；
  **`buy`/`sell` 列互换**（2013–2018 年 30%~75% 行、2019 年 30% 行需修正；`net_buy` 为唯一可靠锚）；
...
- **因子体检 + WF A/B 全链（报告 `docs/top_inst_factor_health.md` / `docs/top_inst_wf_ab_result.md`）：
  ...
  **WF A/B（6 折 8-13，预登记 `docs/plans/top_inst_ab_prereg.md`；数据态与 B20 逐位相同 `511c92f4`；
  ...
  再启仅限新载体（席位身份结构）或新标签域（事件域短窗）且重新预登记。
...
- **政策层 E2 主动减仓（P2-3 v2）**：新增 `src/lazybull/backtest/exposure_trim.py`（`BacktestExposureTrimMixin`，引擎已挂载）。
  语义：**每日**判定 `持仓市值 > λ×组合总值 + EXPOSURE_TRIM_TOLERANCE(3%)`，超配则 T0 生成卖单、
  **T+1 开盘**对**全部持仓按同一比例部分卖出**（不做个股选择——个股截面退出已被 R-002 否定）；
  已在到期/止损整仓队列的股票跳过（整仓优先）；停牌/跌停不进延迟订单队列（该队列按整仓卖出语义重试，
  会把减仓放大成清仓），改为跳过并由次日每日判定重试。与既有**买入预算缩放**（`exposure_override.py`）
  配合构成完整降暴露：减仓降**存量**（实测 λ<1 日敞口均值 52.5%），缩放防**增量**买回。
- **卖出链路支持部分卖出**：`_sell_stock` / `_sell_stock_with_status_check` / `_sell_stock_direct` 新增
  `fraction`（∈(0,1]，默认 1.0 行为不变）与 `allow_pending`；新增 `_resolve_trim_shares`（整手卖出，
  剩余不足一手整仓卖出）；部分卖出保留剩余持仓、每股成本基准不变、`buy_cost_cash` 按剩余比例缩减；
  成交记录新增 `sell_type=risk_trim` + `sell_reason` + `trigger_type`。
- **批量脚本参数化**：`batch_walk_forward.ps1` 新增 `-ExposureTable`（默认空 = 生产不启用）与
  `-SkipCompare`（影子多臂实验省时）；参数经顶部 `param()` 单一来源注入，不再散落变量。
- 测试：`tests/test_backtest_exposure_trim.py`（27 项）覆盖 T0/T1 时序、容差、股数取整、账务、
  延迟队列禁用、**真实卖出链**与**签名契约**（`inspect.signature`）。

### Fixed

- **`_sell_stock` 签名丢失 `allow_pending`**（多次编辑重叠残留）：单测因**桩覆写**入口方法而全绿，
  实际影子回测在首个触发日（2024-01-02）`TypeError` 崩掉。现补签名契约测试 + 走真实 `_sell_stock` 的执行测试。
- **折内异常不会让批量脚本报错**：`walk_forward.py` 折内异常被捕获后仍打印"成功完成: 14 / 14 个切分"、
  ps1 也报"失败 0 个"，但 `chain_nav` 只剩 10 折（4 折无产物）→ 影子臂对比脚本加**折数守卫**
  （臂与基线 `chain_nav` 行数/折数不一致直接报错）。
### 记录（P2-3 v2 四臂影子回测，2026-09-14）

- 臂批次：基线（全 1 表，与不启用**逐位一致**）`wf_batch_20260914_134741`；
  **E2-滚动250 `wf_batch_20260914_142415`**；对照A 纯 regime `wf_batch_20260914_143353`；
  对照B 纯分数 `wf_batch_20260914_144337`（各 14 折完整，窗口 20240102~20251204）。
- **预登记判据通过**：E2 Δ收益 **+0.1730**、ΔMaxDD **+0.0102**、ΔSharpe **+0.3759**，
  **逐折 MaxDD 4/4 改善**；窗口外 split 0~9 三臂与基线**逐位一致**（净值 round(12) 相同）。
- **必须与结论一起读**：① 68% 的收益来自"信号期外滞留"（−未即时回补，+7.14pp / 总 +10.47pp）；
  ② 收益 ≈95% 集中在 2024-01/02 与 2025-04/10（2024-11 为 −3.32pp）；③ 逐折收益仅 2/4 为正。
  跨折稳健的是**回撤与波动**。详表与三条未验证项见 `docs/terminal_loss_risk_register.md` R-004 §8。

## [0.117.0] - 2026-09-14

### Added

- **政策层 E2 shadow 接入（P2-3，暴露覆盖）**：新增 `src/lazybull/backtest/exposure_override.py`（`BacktestExposureOverrideMixin` + `load_exposure_table`），引擎买入预算基数乘上**信号日暴露系数**（调仓与补齐两条路径都乘），并新增 CLI `--exposure-table`（`walk_forward.py`）与 ps1 配置项 `$exposure_table`；门控产物新增 `--export-table`（导出两列 `日期, 暴露系数` CSV）。**语义**：缩减/暂停加仓（降暴露日新买入只投 λ，释放现金闲置，持仓到期照常卖出），真实暴露随换手逐步下降；不改卖出指令、不动 T0/T1 链路、不动现金账务。
- 契约与验证：引擎默认关闭（表为 None）时**逐位一致**（实测单 split：全 1 表 vs 不启用，链式净值 md5 相同、成交仅 `wf_run_id` 不同）；缺日首次出现即告警并按 1.0 处理；非法系数（或 >1）在任何一环都直接报错。测试 `tests/test_backtest_exposure_override.py`（16 项）。

### Fixed

- **导出系数表必须按交易日历补齐台账缺口**：风险台账只覆盖“当日有持仓且可打分”的交易日（实测 2024-01~2025-12 缺 61/466 日 ≈ 13%，最长连续 14 日），直接落盘会让引擎在缺口日按 1.0 兜底（实测单 split 降暴露完全未生效）；现由 `export_exposure_table(..., trading_days=...)` 按日历**顺延前一交易日状态**（不向外推区间外日期）。
- 中文文件名经 PowerShell 组装命令串会被转码（`暴露系数表` → 乱码），引擎输入的系数表改用 ASCII 文件名。

### 记录（P2-3 验证与四臂影子回测，2026-09-14）

- 单 split 验证（split12）：总收益 +2.35% → +5.43%，MaxDD −11.13% → −9.85%（机制生效）。
- **四臂完整 14 折影子回测（窗口 20240102~20251204）**：
  基线 0.3325 / −0.2695 / Sharpe 0.6094；**E2-滚动250 0.3326（Δ+0.0001）/ MaxDD −0.2695（Δ0.0000）/ Sharpe 0.6258**；
  对照A 纯regime 0.3543 / −0.2735 / 0.6945；对照B 纯分数 0.2101 / −0.2695 / 0.4299；E2-固定 0.3294 / −0.2702 / 0.6351。
- **预登记判据不通过**（Δ收益>0 且 ≥2/3 批 MaxDD 改善 且 Sharpe 不下降）：E2 净效应≈0、回撤零改善。
- **根因**：降暴露状态只落在 28 个调仓信号日中的 5 个（17.9%），触发段多为 1–3 日而调仓周期约 20 交易日 →
  动作与信号错配；净值近似因假设“持续 λ 暴露”而**高估 2–3 个数量级**。后续选项（主动减仓 / 终止路线）见 R-004 §7。

## [0.116.0] - 2026-09-14

### Added

- **暴露门控 E2（政策层 P2-2，一阶筛选）**：`src/lazybull/risk/terminal_loss/exposure_gate.py` + 入口 `scripts/calibrate_exposure_gate.py`，把 P2-1 风险台账聚合为日级组合面板，按"校准段拟合、评估段套用"的预登记口径评估三臂（`combined`＝E2：市场波动分位阈值 **且** 层内组合平均风险分位阈值；`score`＝纯模型分数对照；`regime`＝纯 regime 对照）。契约：校准段与评估段重叠直接报错；非 valid 标签行剔除并告警；同日"市场波动状态"不唯一直接报错；层内校准日数低于下限拒绝校准；降暴露系数必须落于 (0, 1]。产物全部中文表头（`common/sidecar_schema.py` 新增表 5/6/7）：`暴露门控校准.csv` / `暴露门控逐日判定.csv` / `暴露门控评估.csv` / `暴露门控评估_按折.csv`。
- 判据按三批实测改写：只看"降低平均亏损日频率与幅度"（`触发日平均加权收益` / `触发日亏损日频率`），**不得**以 MaxDD/尾部损失为判据（层内两半最差单日几乎相同）；`收益项/成本项/净增量（日均口径）` 是**一阶代理**，组合级净值必须由 P2-3 shadow 模式回测给出。
- **滚动分位口径（`--threshold-mode rolling --window-days 250`）**：阈值逐日重算（只用 `[t−W, t−1]` 窗口：regime 阈值 = 窗口波动分位、得分阈值 = 窗口内高波动层的得分分位），不依赖固定校准段；逐日表新增 `阈值口径 / 窗口日数 / 市场波动阈值 / 得分阈值` 四列，两种口径**不得混组比较**。首轮实测（三批）：触发占比 33%→**19–21%**，触发日事件率 21–27%→**30–36%**，触发日平均加权收益 −0.4~−1.4%→**−1.8~−2.3%**；两条对照臂仍为负。
- 测试 `tests/test_exposure_gate.py`（18 项）：三臂触发真值表、阈值=校准段分位、层内样本下限、区间重叠与缺列硬报错、首触标记（折内，跨折重计）、成本与净增量恒等式、中文表头数量一致性、端到端三表产出与分层不重不漏。

### 记录（E2 首轮校准与三臂评估，2026-09-14）

- 口径：校准段 20220701–20231229（320 交易日），评估段 20240101–20251204（405 交易日），λ=0.5，单边成本 15bp，三批相同模型与期间、仅组合规模不同。
- **只有 E2 三批净增量全为正**（+0.00231 / +0.00067 / +0.00096），两条对照臂**全为负**（纯模型分数 −0.00037 / −0.00157 / −0.00179；纯 regime −0.00261 / −0.00299 / −0.00332）；保守的"首触日"口径下 E2 同样三批为正（+0.00008 / +0.00021 / +0.00015），对照 A/B 分别全负与≈0。
- 触发日平均加权事后收益：E2 三批为 **−1.39% / −0.42% / −0.62%**（未触发日 +2.11% / +1.58% / +1.90%）；触发日事件率 26.6% / 20.8% / 22.5% vs 未触发 12.8% / 15.8% / 14.1%。
- 纯 regime 臂触发在**更安全**的日子（事件率 16.3% / 15.6% / 14.9% < 未触发 21.1% / 23.6% / 23.0%）——再次印证"市场波动本身无区分度，模型分数携带独立信息"。
- 成本敏感：单次首触事件毛收益 ≈ 0.5×触发日损失（0.35%–0.50%），往返成本按 2×15bp×λ = 0.15% 同量级 → P2-3 影子运行必须先做成本敏感性；触发区间偏长（139 触发日 / 14 首触 ≈ 10 日/次），首次触发后按"持有到触发结束"实现时，真实暴露窗口需在 P2-3 明确。
- **量纲校正（同日补测）**：台账事后收益是持有期口径（最长 20 交易日、逐日行重叠），`收益项×交易日数` 不能读作净值幅度（top_n=20 全部触发日口径折合 +39%，净值近似仅 +1.4pp）——**一阶代理只能读方向与排序，不能读量级**。

### 记录（E2 组合级净值近似，2026-09-14）

- 区间 2024-01-02~2025-12-04（402–405 交易日，λ=0.5，切换按 λ×15bp 计成本；忽略释放资金对后续调仓的影响）：
  Δ收益 **+0.62pp / −4.74pp / +1.39pp**（三批符号不稳定），**ΔMaxDD = 0.00pp（三批完全零改善）**，切换成本 ~2pp。
- 事实（不依赖近似）：**最大回撤段 split11（2024-06-11~2024-12-11）触发日 = 0**——该段 57 个高波动日，但组合平均风险分数低（2024H2 折均值 0.138 为全折最低）；触发集中在 2024-01 崩盘月（11 日，有益）与 2025-02~2025-11 上涨行情（202502 17/17、202508 20/20 日），split13 损失 −8.4pp。
- 根因：模型水平随 regime 漂移（折级 ρ(折均值得分, 折事件率)=−0.61），固定绝对阈值在“模型系统性高估风险”的时期触发、低估期不触发。登记为 **R-004（accepted）**，含三条缓解路径（滚动分位阈值 / 先修校准漂移 / 终止路线），且明确 **P2-3 不得按当前形态直接接入**。

### 记录（缓解路径① 滚动分位口径，2026-09-14）

- 信号层面三批一致变锐利（见上）；净值近似（`m_t=λ` 缩放）在两种执行假设下均为正：
  乐观 T+1 **+21.2 / +24.1 / +22.1pp**、保守 T+2 **+9.8 / +12.0 / +10.3pp**（ΔMaxDD 0~+1.3pp）。
- **乐观口径含开盘跳空伪影**（降暴露状态覆盖 2025-04-07 −8.8%、2024-10-09 −8.4% 等最差日，而 T+1 开盘卖出躲不过当日跳空）→ 幅度**不可当预期收益**，只能读方向；R-004 状态更新为 **mitigated-pending-shadow**，P2-3 影子回测为唯一验收口径。

## [0.115.0] - 2026-09-14

### Added

- **政策旁路批量入口 `scripts/batch/batch_policy_sidecar.ps1`（P2-1）**：按 **WF batch**（`data/walk_forward/batches/<batch>/raw/walk_forward_持仓快照_*.csv`）选择快照打分，替代“扫 raw 目录全部文件”；参数 `-Batch`（latest / 目录名 / 完整路径）、`-Arms`（多特征集循环）、`-PHi` / `-PAbs`（阈值网格）、`-NoDayPct` / `-NoMissed`、`-ListBatches`（列出各 batch 的 summary 与持仓快照数量、“可打分/无持仓快照”标记）、`-DataRoot` / `-RiskRoot` / `-OutDir`。产物落 `<batch>\policy_sidecar<arm>\{风险台账,触发清单,阈值扫描}.csv`（中文表头），运行日志落仓库根 `logs\`（与 v0.114.1 日志约定一致）；旧 batch 无快照时给出明确指引（重跑 WF batch 或 `--skip-training` 复用已注册模型）。

### Fixed

- **政策旁路打分链路两处（端到端跑真实 batch 时发现）**：① 折模型加载——固定名别名 `terminal_loss_model.joblib` 是 payload dict，必须经 `TerminalLossModel.load` 还原（原先直接 `joblib.load` 会得到 dict 并报 `AttributeError`；`v{N}_model.joblib` 为模型实例时直接返回）；② 标签网格**按交易日分块构建**（`LABEL_CHUNK_DAYS = 60`，块尾按 `h_max + 1` 多切保证端点落在真实数据），多年 OOS 上一次生成全网格会达上亿行、内存不可接受。
- **性能**：cs_train 分区改为按需列读取（先读 parquet schema 校验、再只取所需列），避免每次整表 383 列；缺列依旧明确报错（不静默降级）。
...
- 变量：同一批模型（24008–24021）、同一期间与折集合，仅 `bt_top_n` 10 → 20（两批 `stagger_tranches` 均为 1）。
- 持仓域：12829 valid 行（首次出现 12 行 `sigma_unavailable`、1 行 `endpoint_missing`）/ 726 交易日 / 日均持仓 17.67。
- **个股退出仍不可用（两批一致）**：AUC 0.5866 → 0.5662；日内分位 lift 1.24 → 1.34；可避免损失合计仍为负（−6.30 @0.95/0.10）；`P_abs` 条件不 binding（0.05 与 0.10 结果逐位相同）。
- **“组合级水平有信息”未复制**：日级 ρ(日均风险概率, 当日事件率) +0.2186 → **+0.1061**；日均分半事件率比值 2.072 → **0.992**；月级 ρ +0.022 → −0.099。仅极端尾部（q≈0.95）仍保留 23.5% vs 15.9% 事件率，但触发组收益也更低（+0.16% vs +1.05%）。已登记为 **R-003（已接受）**。
- 主策略（仅作附带观察）：分折 6/14 胜，均值年化 22.0% → 18.7%、中位数 15.2% → 20.4%，MaxDD 均值 −14.10% → −13.95%，最差窗口 split10（2023-12~2024-06）+5.5% → −3.9%。分散化未降低回撤。

### 记录（P2-1 第三批复核与 R-003 裁决，2026-09-14，`wf_batch_20260914_091148`）

- 变量：同一批模型、同一期间、`stagger=1`，仅 `bt_top_n=15`（三批 10 / 15 / 20）。
- 三批日级 ρ = **+0.219 / +0.132 / +0.106**（单调随规模下降，90% CI 两两重叠），分半事件率比 2.07 / 1.19 / 0.99 → 按 R-003 复审条件判定 **“无条件日均可得分阈值”路线终止**。
- 分歧根因已定位（三批一致）：跨折 ρ(折均值得分, 折事件率)=−0.61（模型水平不随 regime 上移）与折内 ρ=+0.369/+0.269/+0.216（随规模递减）混合；汇总值随组合变大而收敛到 0/负。
- **可复制条件结构**：`市场波动 ≥ 上 1/3` × `当日组合平均风险高` → 层内事件率比 **3.06 / 1.96 / 1.77**、收益差 −3.8pp / −3.4pp / −3.0pp；2×2 中仅该格为坏格（事件率 21.6–27.4%、收益负）；**对照臂显示纯市场波动分层与层内市场波动排序均无效**（0.87–1.04）→ 模型分数携带独立信息。保留为 P2-2 候选（E2 设计见计划 §7.5，必须带两条对照臂）。
- 主策略三档：均值年化 22.0% / 21.4% / 18.7%，中位 15.2% / 17.2% / 20.4%，Calmar 1.57 / **1.69** / 1.54，MaxDD 均值 −14.10% / **−13.72%** / −13.95%。
...
- **钉钉 model 命令输出改为精选关键信息**：`format_model_info` 不再全量展开 `train_params`（可能内嵌大清单/诊断字段），改为按白名单（`_MODEL_INFO_KEY_PARAMS`：algorithm/objective/n_estimators/best_iteration/max_depth/learning_rate/正则系数/tree_method/device 等 22 项）展示关键超参，白名单外的参数仅给省略计数；白名单内单值超 80 字符截断展示。`MODEL_INFO_MAX_CHARS=2000` 总长上限仅作防御兜底，正常输出约 900 字符。已用真实 v19881 metadata 端到端验证：输出 913 字符完整信息卡片。
- **钉钉 model 命令按版本回退旁路元数据**：注册表 `model_registry.json` 存在但**不含** config 指定版本时（如容器内 registry 为陈旧整包、实际使用模型仅有 `v{N}_metadata.json` 旁路），此前直接返回"未找到版本 X 的模型"并列出全部可用版本（数百个版本号再次撑爆消息）；现在按版本两段查找——注册表优先、不含该版本时回退旁路元数据文件；未指定版本时以 `latest_model_version.txt`（`get_latest_version`，含 classifier 过滤）为准。未找到时的可用版本提示改为去重合并两处来源、只展示最近 10 个并给出总数（`可用版本共 N 个，最近 10 个: [...]`）。新增"registry 不含指定版本回退 sidecar""数百版本号提示截断""白名单精选""单大值截断"等测试（`tests/test_paper_reporting.py`）。
...
- **质量覆盖口径**：覆盖率以显式扫描区间或本地 `raw/daily` 最新分区为截止，不再由当前日期或交易日历外推；支持各数据层有效起点，并为 `cs_train` 的 T+1+20 标签保留 21 个交易日尾窗。
- **质量缺失率误报**：逐分区缺失率改为报告信息，错误门禁使用扫描区间行数加权缺失率；默认仅对特征数据集的全空列强制失败，raw 可选字段不再误报，关键训练列仍可配置独立阈值。
- **合法高 PE 误报**：移除 `pe_ttm > 1000` 固定异常阈值，避免近零正利润公司的合法供应商原值持续告警。
- **换手率风险因子链路**：在风险因子批量预计算前将 `daily_basic.turnover_rate` 并入复权日线，恢复 `turnover_cv_20` 与 `turnover_percentile`。
- **现金流利润比状态**：`ocf_to_profit` 按自身最近有效公告状态维护，Q1/Q3 未披露 `net_profit` 时不再覆盖上一份有效年报/中报比率。
- **北向休市缺口**：仅对源数据区间内部的港股休市日写入中性市场级因子，源数据尾部之后仍保持缺失以暴露真实下载滞后。
- **无生产者风险列**：移除从未有上游数据的 `short_sell_ratio_5` 注册项，保留实际生产并使用的 `short_balance_change_5`。
- **分红支付率依赖门禁**：离线构建与纸面 ensure 在缺少有效 income 合并年报时立即失败，不再保存全空支付率；下载 CLI 帮助补充 `income`，仅股息率实验清单同步排除支付率。

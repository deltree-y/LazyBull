...
| 字段名 | 类型 | 说明 | 用途 |
|--------|------|------|------|
| filter_is_st | int | ST股票标记，1=是，0=否 | 用于过滤（已过滤，保留字段用于审计） |
| filter_list_days | int | 上市天数（自然日） | 用于过滤（已过滤，保留字段用于审计） |
| filter_suspend | int | 停牌标记，1=停牌，0=正常 | 用于过滤（已过滤，保留字段用于审计） |
| limit_up | int | 涨停标记，1=涨停，0=非涨停 | 不过滤，但需注意涨停后流动性问题 |
| limit_down | int | 跌停标记，1=跌停，0=非跌停 | 不过滤，但需注意跌停后流动性问题 |

...
- `filter_` 前缀已从输出列中移除
- filter 列现在只包含 `is_st` 和 `suspend` 两个字段

| 字段名 | 类型 | 说明 | 用途 |
|--------|------|------|------|
| is_st | int | ST股票标记，1=是，0=否 | 过滤标记（已过滤样本，保留字段用于审计） |
| suspend | int | 停牌标记，1=停牌，0=正常 | 过滤标记（已过滤样本，保留字段用于审计） |
| list_days | int | 上市天数（自然日） | 信息字段，不作为过滤标记 |
| limit_up | int | 涨停标记，1=涨停，0=非涨停 | 标记字段，不过滤，但需注意流动性 |
| limit_down | int | 跌停标记，1=跌停，0=非跌停 | 标记字段，不过滤，但需注意流动性 |

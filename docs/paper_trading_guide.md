...
- **等权一手可买约束（v0.3.5）**：等权模式下自动跳过价格过高的股票并顺延补足，确保所有目标股票至少可买1手（100股）
...
在使用等权策略（`weight_method=equal`）时，系统会自动检查每只候选股票是否能以当前资金分配买入至少1手（100股）。如果某只股票价格过高导致无法买入1手，系统会：

1. 跳过该股票
2. 从排序候选列表中顺延选择下一只股票
3. 重复此过程直到凑足 `top_n` 只可买股票或候选列表耗尽

### 工作原理

**价格判断：** 使用 T1 的买入价格类型（`buy_price_type`：open 或 close）计算可买股数

**计算公式：**
```
等权分配金额 = 总资金 / top_n
可买股数 = int(等权分配金额 / 股票价格 / 100) * 100
```

**过滤条件：** 可买股数 < 100 的股票将被跳过
...
- 总资金：100,000 元
- top_n：3 只
- 等权分配：100,000 / 3 = 33,333 元/只

候选排序（按ML预测分数）：
1. 000001.SZ - 价格 1200 元 → 可买 27 股（不足1手，跳过）
2. 000002.SZ - 价格 50 元 → 可买 666 股（6手，选中）
3. 000003.SZ - 价格 30 元 → 可买 1111 股（11手，选中）
4. 600000.SH - 价格 25 元 → 可买 1333 股（13手，选中）

**最终结果：** 选择 000002、000003、600000 三只股票，等权分配（各 1/3）
...
```
等权+一手约束: 原始排序候选数 50
等权+一手约束: 最终目标数 3, 跳过 2 只 (原始候选 50)
...
- **仅对等权策略启用**（`weight_method=equal`）
- Score 加权策略（`weight_method=score`）保持原有行为

### 注意事项

1. 该约束在 T0 生成信号阶段应用，保存到 pending 的目标已是过滤后的结果
2. 最终目标数量可能小于 top_n（当可买候选不足时）
3. 与回测引擎的行为对齐，确保纸面交易和回测结果一致

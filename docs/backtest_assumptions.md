...
**当前实现**:
- **交易时机**: T 日生成信号，T+1 日收盘价买入，T+n 日收盘价卖出
- **持有期**: n 由调仓频率决定（日频=1天，周频=5天，月频=20天），或可自定义
- **假设**: 可以在收盘价附近完成所有交易

**交易流程**:
1. T 日：根据当日特征生成选股信号
2. T+1 日：以 T+1 日收盘价买入信号中的股票
3. T+n 日：以 T+n 日收盘价卖出所有持仓（n 为持有期）

**边界处理**:
- 回测期末尾不足 T+1 或 T+n 的信号将无法完成交易
- 如果 T+1 或 T+n 日停牌，将跳过该股票的交易

...
def compute_daily_rankic(predictions: pd.Series, true_returns: pd.Series) -> float:
...
def compute_daily_topk_returns(
    predictions: pd.Series, true_returns: pd.Series, k_values: List[int]
) -> Dict[str, float]:
    """计算单日 TopK 平均收益
...
def evaluate_predictions_by_date(
    ...
    for trade_date in df[date_col].unique():
        ...
        # 合并指标
        metrics = {date_col: trade_date, "RankIC": rank_ic, "样本数": len(predictions)}
...
def summarize_daily_metrics(daily_metrics: pd.DataFrame) -> Dict[str, float]:
    ...
    # TopK 收益统计
    topk_cols = [
        col for col in daily_metrics.columns if col.startswith("Top") and col.endswith("平均收益")
    ]
...
def compute_diagnostic_statistics(
    df: pd.DataFrame, date_col: str, prediction_col: str, return_col: str, topk_values: List[int]
) -> Dict[str, Any]:

...
def compute_pledge_high_flag(
    ...
    **kwargs,
    ...
    """质押率分档编码（pledge_ratio 为百分比口径 0-100）：
    >50% → 1（高危），30-50% → 0，<30% → -1。
    ...
    result[raw_ratio > 50] = 1
    result[raw_ratio < 30] = -1
...
def compute_pledge_delta(
    ...
    **kwargs,
    ...
    """质押率 Delta-on-Update（百分比口径 0-100）：仅公告日有变化时非零。
